...
def main(name_of_model, number_of_training_or_testing_observations, path_to_data_frame):
    ...
    for pair in dictionary_of_numbers_of_standard_deviations_below_mean_and_colors.items():
        ...
        df['cumulative_frequency_of_indicators_that_observed_response_value_is_below_threshold_for_trained_model'] = np.cumsum(df['indicator_that_observed_response_value_is_below_threshold'])
        df.drop('indicator_that_observed_response_value_is_below_threshold', axis = 1, inplace = True)
        df['cumulative_relative_frequency_of_indicators_that_observed_response_value_is_below_threshold_for_trained_model'] = df['cumulative_frequency_of_indicators_that_observed_response_value_is_below_threshold_for_trained_model'] / max(df['cumulative_frequency_of_indicators_that_observed_response_value_is_below_threshold_for_trained_model'])
        df.drop('cumulative_frequency_of_indicators_that_observed_response_value_is_below_threshold_for_trained_model', axis = 1, inplace = True)
        print(df.head(n = 3))
        plt.plot(df.index, df['cumulative_relative_frequency_of_indicators_that_observed_response_value_is_below_threshold_for_trained_model'], label = f'trained model; z score = {z_score}', color = pair[1])
    plt.grid()
    plt.legend()
    plt.title(f'Cumulative Relative Frequency\nOf Indicators That Observed Response Value Is Below Threshold Determined By z Score\nVs. Index\nIn Data Frame Of {number_of_training_or_testing_observations} Indicators Sorted By Average Of Predicted Response Values\nFor {name_of_model}', fontsize = 10)
    plt.xlabel('Index')
